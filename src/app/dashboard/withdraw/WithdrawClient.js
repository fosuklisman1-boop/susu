...
export default function WithdrawClient({ availableBalance, userEmail, groupId, groupName, isGroup }) {
      ...
      <div style={{ padding: '20px' }}>
        ...
        {state?.success ? (
          ...
          <form action={action} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
            ...
            <div style={{ background: 'white', borderRadius: '16px', padding: '20px', boxShadow: '0 4px 12px rgba(0,0,0,0.03)' }}>
              <label style={{ display: 'block', fontSize: '0.85rem', fontWeight: '600', color: 'var(--text-muted)', marginBottom: '12px' }}>
                {method === 'Mobile Money' ? 'Mobile Money Details' : 'Bank Account Details'}
              </label>
              <textarea 
                name="payout_details" 
                required
                placeholder={method === 'Mobile Money' ? 'e.g. MTN MoMo - 0244123456 (Kwame Mensah)' : 'e.g. Zenith Bank - 1012345678 (Kwame Mensah)'}
